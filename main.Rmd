...
### Prueba algoritmo 
Requiere cambiar los returns para solo obtener los índices de los clusters
```{r}
t <- proc.time()
clusters <- 5
res5 <- kmeans_par(10, clusters, mdl_4, 5)
proc.time() - t
```
...
### Obtener el set de clusters moda

```{r}
vec <- as.vector(sapply(data.frame(res5), function(x) x[[2]]))
...
## Obtención de la clusterización óptima
```{r}
...
```{r}
metrica$x <- 1:10
ggplot(metrica, aes(x = x)) + 
  geom_line(aes(y=mdl1)) + 
  geom_line(aes(y=mdl2)) +
  geom_line(aes(y=mdl3)) +
  geom_line(aes(y=mdl4), color = "blue") +
  theme_minimal() + 
  labs(title = "Métrica WCSS", 
       x = "clusters",
       y = "WCSS")
```
