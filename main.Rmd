...
kmeans <- function(nodes, df){
  ...
  numiter <- 0
  while(numiter < 100){
    clusters <- sapply(1:len, function(x) colocar(val[x,],centr))
...
t <- proc.time()
clusters <- 6
prueba <- kmeans(clusters, mdl_3)
proc.time() -t
```

## Resumen clusters
```{r}
sapply(1:clusters, function(x) colSums(mdl_3[prueba == x,7:16]))
```

```{r}
sapply(1:clusters, function(x) sum(prueba == x))
```
