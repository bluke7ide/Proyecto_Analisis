...
# Clusterización por medio de métodos vistos en clase (kmeans)
...
## DBSCAN
```{r}
# Ajuste de DBSCAN (eps y minPts se eligen con juicio o gráfico kNNdistplot)
db <- dbscan(modelo, eps = 0.5, minPts = 10)

# Clusters encontrados
table(db$cluster)

# Visualización
# plot(modelo[1:2], col = db$cluster + 1L, pch = 20)
```
...
# Crear la grid y entrenar SOM
som_grid <- somgrid(xdim = 5, ydim = 5, topo = "hexagonal")
som_model <- som(X, grid = som_grid, rlen = 100)

# Visualización
plot(som_model, type = "mapping", main = "SOM Clustering")
```
...
## Spectral Clustering
```{r}
sc <- specc(as.matrix(modelo), centers = 3)

# Resultados
table(sc)

# Visualización
plot(modelo, col = sc, pch = 20)
```
...
```{r}
teigen_model <- teigen(modelo, )

# Resultados
table(teigen_model$classification)
```
...
# 3. Ejecutar Mean Shift
#    bandwidth es el parámetro de ventana; prueba varios valores (0.5, 1, 1.5, ...)
ms_res <- meanShift(X)

# 4. Obtener clusters asignados
clusters_ms <- ms_res$assignment

# 5. Ver resultados
print(table(clusters_ms))
```
