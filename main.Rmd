...
```{r}
centr <- sapply(1:9, function(x) sapply(1:10, function(y) median(modelo[clusters[,6] == x,y])))
```

```{r}
dist <- sapply(1:9, function(x) 
  sapply(1:length(modelo[clusters$som == x,1]), 
         function(y) sum((modelo[clusters$som == x,][y,] - centr[,x])^2)))
sapply(1:9, function(x) which(dist[[x]] == min(dist[[x]])))
```
