...
- kmeans en el primero
- meanshift en el segundo
- jerarquico en el tercero

Note que el jerárquico y meanshift ajustan de primeros en otras métricas, pero nos interesa una mejor elección en todas

```{r}
(resumen %>% slice(-c(1,2,3,4,5,9)))
```

Por último, se descarta
- spectral por ser el peor en el primero y tercero 
- teigen por ser el peor en el segundo

Por lo tanto, el modelo som, o Self Organizing Maps es el escogido
```{r}
sapply(1:9, function(x) colSums(cat_mdl[clusters$som == x,7:16]))
```

```{r}
sapply(1:9, function(x) sum(clusters$som == x))
```

```{r}
plot(som(scale(modelo), somgrid(xdim = 3, ydim = 3, topo = "hexagonal"), rlen = 100))
```
